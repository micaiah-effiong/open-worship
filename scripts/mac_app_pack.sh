...
cp $APP_BUILD/bin/openworship $APP_EXE_DIR/openworship-real
cp $APP_BUILD/bin/launcher.sh $APP_EXE_DIR/openworship
cp $APP_BUILD/bin/gdk-pixbuf-query-loaders $APP_EXE_DIR
chmod 766 "$APP_EXE_DIR/gdbus"
chmod 766 "$APP_EXE_DIR/openworship"
chmod 766 "$APP_EXE_DIR/openworship-real"
...
echo "Code signing"
find "openworship.app" -depth -type f -exec codesign --force --deep --preserve-metadata=entitlements,requirements,flags,runtime --entitlements Entitlements.plist -s - {} \;
